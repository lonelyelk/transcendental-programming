...
u = ->(ph){k=d*2;k.times.each_with_object({}){|j, o|
  x = r*Math.cos(pi*4*j/k+ ph);
  y = (j<k/32||j>=31*k/32)&&0.31 - 0.24*Math.cos(pi*8*j/k);
  y||=j>=k/32&&j<15*k/32&&0.42 * Math.sin(pi*4*j/k);
  y||=j>=15*k/32&&j<17*k/32&&0.24 * Math.cos(pi*8*j/k) - 0.31;
  y||=-0.42 * Math.sin(pi*4*j/k);
  y *= r;
...
l = ->(fr,ph,s=true){k=d*fr;k.times.each_with_object({}){|j, o|
  x = r*Math.cos(pi*2*j/k * fr + ph);
  y = r*0.42*Math.sin(pi*2*j/k);
  o[[x.round,y.round]] = true;
  o[[-x.round,y.round]] ||= s;
}};
...
c.times{|i|o[u[pi/2 - 4*pi*Math.sin(pi*i/c/2)/10]]};
c.times{o[b]};
z=->(x,y,m){m&&s||a[y][x]};
o[b];
print("#{e}[#{h}B")
